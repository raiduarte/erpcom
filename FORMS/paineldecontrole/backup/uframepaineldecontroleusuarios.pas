...
uses
  ...
  Graphics, DBGrids, StrUtils, ZDataset, Dialogs, ComCtrls, Types;
...
type
  ...
  TFramePainelDeControleUsuarios = class(TFrame)
    btnAddicionarPermissao: TSpeedButton;
    btnCancelar: TSpeedButton;
    btnRemoverPermissao: TSpeedButton;
    btnExcluir: TSpeedButton;
    btnSalvar: TSpeedButton;
    btnVoltar: TSpeedButton;
    dbgPermissoesDoUsuario: TDBGrid;
    dbgPermissoesDisponiveis: TDBGrid;
    ...
    procedure btnRemoverPermissaoClick(Sender: TObject);
    procedure btnAddicionarPermissaoClick(Sender: TObject);
...
begin
  ...
  PERMISSOES_DE_USUARIO.Close;
  PERMISSOES_DISPONIVEIS.Close;
end;
...
begin
  btnAddicionarPermissao.Enabled:=(dtsProgsDisp.DataSet.RecordCount>0) and
...
procedure TFramePainelDeControleUsuarios.dtsProgsUsuarioDataChange(
  Sender: TObject; Field: TField);
begin
  btnRemoverPermissao.Enabled:=(dtsProgsUsuario.DataSet.RecordCount>0) and
...
//    PROGS_USUARIO.ParamByname('usuarioID').AsInteger:=dtsUsuarios.DataSet.FieldByName('usuarioID').AsInteger;
    PERMISSOES_DE_USUARIO.Open;
    PERMISSOES_DISPONIVEIS.ParamByName('usuarioID').AsInteger:=USUARIOS.FieldByName('usuarioID').AsInteger;
    PERMISSOES_DISPONIVEIS.Open;
...
begin
  ...
  AtualizaGridEsquerdo(AtualizaListaDeIDs(dbgPermissoesDoUsuario.DataSource.DataSet.FieldByName('usuarioID').AsInteger));
...
begin
  ...
  programaID:=dbgPermissoesDisponiveis.DataSource.DataSet.FieldByName('programaID').AsInteger;
